...
def input_days_and_span_selection() :
    ...
    for combination in tqdm(input_days_span_combination) :
        ...
        for date in dates_set :
            ...
            start_date = date[0]
            end_date = date[1]
            train(start_date, end_date, "ema", input_days, span, 14)
            temp_result.append(test(start_date, end_date, "ema", input_days, span, 14))
...
def moving_average_selection() :
    ...
    for moving_average in tqdm(available) :
        temp_result = []
        for date in dates_set :
            start_date = date[0]
            end_date = date[1]
...
def n_components_selection() :
    ...
    for n in range(1,30):
        ...
        for date in dates_set :
            start_date = date[0]
            end_date = date[1]
            train(start_date, end_date, "ema", 1, 3, n)
            temp_result.append(test(start_date, end_date, "ema", 1, 3, n))
